...
impl ICharacterBody2D for Player {
    ...
    fn ready(&mut self) {
        ...
        let size = config.get_value(GString::from("ui"), GString::from("size")).to_string().parse::<f32>().expect("Failed to parse to f32");
        let opacity = config.get_value(GString::from("ui"), GString::from("opacity")).to_string().parse::<f32>().expect("Failed to parse to f32");
